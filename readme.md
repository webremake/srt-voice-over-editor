...
> [!TIP]
> **Подробная техническая информация** о работе алгоритмов, стратегии ретраев и пайплайне данных доступна в файле [DOCS.md](docs.md).
...
3. **Редактирование**:
   ...
   - **Двухуровневая логика ретраев**: При несоответствии количества строк в ответе — скрипт сначала автоматически пробует обработать участок батчами по **10 строк**, а если и они не проходят — переходит к батчам по **5 строк**.
4. **Rate Limiting**: делает паузу 15 секунд между запросами (включая ретраи) для соблюдения лимитов Free Tier (5 RPM).
5. **Сжатие**: модель удаляет лишние слова, вводные конструкции и разговорный мусор, сохраняя при этом все имена собственные и бренды.
